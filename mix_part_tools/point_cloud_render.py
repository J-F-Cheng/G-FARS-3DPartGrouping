...
def pre_render_process(pts, conf):
    ...
    for i in range(pts.size(0)):
        pts_colors.append(torch.tensor(colors[i%len(colors)], dtype=torch.float, device=conf.device).repeat(pts.size(1), 1))
    return pre_pts.to(conf.device), torch.cat(pts_colors, dim=0)

def pre_render_process_stack(pts, conf):
    """
        pre-rendering processing
        input: pts P x N x 3
        output: processed pts and the corresponding colours
    ...
    for i in range(pts.size(0)):
        pts_colors.append(torch.tensor(colors[i%len(colors)], dtype=torch.float, device=conf.device).repeat(pts.size(1), 1))
    return pre_pts.to(conf.device), torch.stack(pts_colors, dim=0)
...
def point_cloud_render(out_fn, pts, conf):
    ...
    output_img = images[0, ..., :3].detach().cpu().numpy()
